...
class CourseRetrievalEngine():
    ...
    def __init__(self):
        self.dv = Doc2Vec.load('recommender/nlpmodels/doc2vec_model')
        self.tf = pickle.load(open('recommender/nlpmodels/tfidf_model.pkl', 'rb'))
        self.svd = pickle.load(open('recommender/nlpmodels/svd_model.pkl', 'rb'))
        self.svd_feature_matrix = pickle.load(open("recommender/nlpmodels/lsa_embeddings.pkl", 'rb'))
        self.doctovec_feature_matrix = pickle.load(open('recommender/nlpmodels/doctovec_embeddings.pkl', 'rb'))
        #self.df = df = self.get_df()
        self.df = df = Course.objects.all()
        self.hal = sia()
    ...
    def view_recommendations(self, recs):
        ...
        for i in range(len(recs)):
            ...
            print("single_title" + single_title)
            #single_course = self.df.('id==@single_title')
            single_course = Course.objects.get(number = single_title)
            number = single_course.number
            ...
            rec_courses.append({'number': number, 'name' : name, 'description' : description})
